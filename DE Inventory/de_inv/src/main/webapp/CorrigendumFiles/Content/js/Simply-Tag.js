...
var delcarationAutoSuggest = function ($) {
    ...
    var _addItemTotagList = function (placeHolder, key, value, cssStyle) {
        ...
        var ctrlId = $(placeHolder).attr('id'); //......Id of parent.
        //add if not exist
        if (existingValue.length === 0) {
            ...
            placeHolder.siblings('span.simply-tags').append("<span data-key='" + key + "' data-display='" + value + "' class='tag label label-info " + cssStyle + "'>"
                + value +
                 "<span data-role='remove' id='" + ctrlId + "spanR_" + key + "' title='Remove' onClick=onTagClosed('" + ctrlId + "','" + key + "')></span></span>");
            placeHolder.parent().find(".tag [data-role='remove']").on('click', function () {
                $(this).parent().remove();
            });
            onTagAdded(ctrlId, key);
        } else {
            existingValue.fadeIn(200).fadeOut(200).fadeIn(200);
        }
        //..onTagClosed/onTagAdded.This function is created by Avi and its definition should be 
        //in project where we can set logic of what need to be done (Like manage id variable, add class etc ) 
        //when ever any new tag is added or removed.
        //id='" + ctrlId + "spanR_" + key + "' this is id of span which is unique so that can be handle uniqly 
        placeHolder.siblings('.simply-tag-suggestion-root').css('display', 'none');
    ...
    var _displaySuggestions = function (placeHolder, settings, text) {
        ...
        var ctrlId = $(placeHolder).attr('id');
        var classSelected = '';
        placeHolder.siblings('.simply-tag-current').attr('data-type', 'invalid');
        ...
        if (settings.dataSource.length != 0) {
            $.each(settings.dataSource, function (index, dataSource) {
                ...
                if (dataSource[settings.value].toLowerCase().includes(text.toLowerCase())) {
                    matchCount += 1;
                    if ($('#' + ctrlId + 'spanR_' + dataSource[settings.key]).length > 0)
                        classSelected = 'simplyselectedItem';
                    suggestionHtml += " <div class='simply-tag-suggestion " + classSelected + "' id='divS" + dataSource[settings.key] + "'  data-value='" + dataSource[settings.value] + "' data-key='" + dataSource[settings.key] + "' >" + dataSource[settings.value] + "</div>";
                }
