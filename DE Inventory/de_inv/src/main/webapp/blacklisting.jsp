...
<!-- 	<div class="container-fluid"> -->
	<div class="container">
		...
		<h4>BLACKLISTING DETAILS</h4>
		
		<hr class="impo-bottom-border">
		<div class="tablediv">
			<table class="display" id="newsblog"  class="cell-border" style="width:100%">
				<thead style="background-color: #f09554; border: 2px">
					<tr>
						<th>SNo.</th>
						<th>Particulars</th>
						<th>Firm Name</th>
						...
						<th style="width: 25%">Order No(Order Copy)</th>
						<th>Date of Blacklisting</th>
						<th>Duration of Blacklisting</th>
				...
				<tbody>
					...
					<%
                                  int i=1;
                              ResultSet nsq_rs_all = null;
                              ResultSet t_rs_amend = null;
                              GetConnection Conn = new GetConnection();
                        try
                        {
                            DaoAdmin da = new DaoAdmin();
                            nsq_rs_all = da.nsqreportall();
                                
                                if(!nsq_rs_all.isBeforeFirst())
                                {
                                    %>
					<tr>
						<td colspan="3">
					...
					<tr>
						...
						<td>
							<%out.print(nsq_rs_all.getString("particular")); %>
						</td>
						<td>
							<%out.print(nsq_rs_all.getString("firm_name")); %>
						</td>
						<td style="width: 25%">
							<%out.print(nsq_rs_all.getString("firm_address")); %>
						</td>
						<td style="width: 20%">
							<%out.print(nsq_rs_all.getString("nsq_reason")); %>
						</td>
						<td style="width: 10%"><a target="_blank"
							href="<%out.print(nsq_rs_all.getString("nsq_file"));%>"><!-- <i
								class="fas fa-file-pdf"></i> --><%out.print(nsq_rs_all.getString("nsq_orderno"));%></a></td>
						<td style="width: 15%">
							<%out.print(nsq_rs_all.getString("nsq_date")); %>
						</td>
						<td style="width: 15%">
							<%out.print(nsq_rs_all.getString("nsq_duration")); %>
						</td>
						<td style="width: 15%">
							<%out.print(nsq_rs_all.getString("nsq_remarks")); %>
						</td>
					...
					<%
                        ...
                        }catch(Exception e){out.print("hi"+e);}   
                        finally{
                        	/*  t_rs_amend.close();
                             t_rs_all.close(); */
                        }
